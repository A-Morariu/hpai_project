...
def toy_log_ll(data):
    ...
    def target_log_prob(parameters):
        ...
        prior_dist = tfp.distributions.Normal(
            loc=0., scale=10.).log_prob(parameters)
...
def random_walk_metropolis_hastings_kernel(target_log_prob_fn, proposal_scale):
    ...
    return bootstrap_result, random_walk_metropolis_hastings_one_step


def sample_chain(
        current_state, previous_kernel_results, kernel, num_results=15):
...
# Run the algorithm - should run faster than the notebook so this is the next to do so we can debug here
